...
static void decodeStr(CString cryptokey, CString& strInOut)
{
	...
	strInOut = res.c_str();
	strInOut.TrimRight(); // избавляемся от лишних пробелов, которые могут появиться в результатах подтасовки
...
static void encodeStr(CString cryptokey, CString& strInOut)
{
	if (cryptokey.IsEmpty() || strInOut.IsEmpty())
		return;
	// идея такая. нужно проверять возможность раскодирования 
	// (пароль: test123 текст: 138500  - это не расшифровывается. но, если к тексту добавить пробел, все ок)
	// естественно, это косяк кода шифрования, но разбираться лениво
	CString etalonStr = strInOut;
	CString decodedStr = _T("");
	int index = 0;
	while (etalonStr.Compare(decodedStr.GetString()) != 0){
		if (index == 10) {
			CString errMsg; errMsg.Format(_T("Не вдалося зашифрувати \"%s\".\nДану інформацію зберегти не вдасться."), etalonStr.GetString());
		...
		CString codedStr = strInOut; // тут лежит закодированная строка
		LocalFree(dst);
		delete[] src;
		_tsetlocale(LC_ALL, _T("C"));
