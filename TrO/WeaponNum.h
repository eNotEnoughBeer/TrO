...
public:
	...
	void Read(Excel::_WorksheetPtr sheet, CString key) {
			...
			for (i = 0; i <= iUBound[0] - iLBound[0]; i++)
			{
				for (j = 0; j <= iUBound[1] - iLBound[1]; j++)
				{
					...
					{ // Если поле имеет недопустимый формат
						CString s; s.Format(L"Помилка читання WEPNUM. Код: 0x%8X %s [%d, %d]", er.Error(), er.ErrorMessage(), i, j);
		...
		{
			CString s; s.Format(L"Помилка читання WEPNUM. Код: 0x%8X %s [%d, %d]", err.Error(), err.ErrorMessage(), i, j);
			AfxMessageBox(s, MB_ICONERROR);
			weaponNums.clear();
		}
