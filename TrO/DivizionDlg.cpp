...
void DivizionDlg::OnBnClickedDone2()
{
	...
	if (res > 0) {
		AfxMessageBox(_T("Виникли проблеми із каталогом військового підрозділу"));
	}
...
void DivizionDlg::parseDivision(int rotaId, int vzvodId)
{
	...
	// выбираем все взводы для конкретной роты. если rotaId==0, выбираем взводы для первой роты в списке rota 
	if (rotaId == 0) {
		if (!rota.empty()) {
			rotaId = rota[0].id;
		}
	...
	// выбираем все отделения для взвода. если vzvodId==0, выбираем отделения для первого взвода в списке vzvod
	if (vzvodId == 0) {
		if (!vzvod.empty()) {
			vzvodId = vzvod[0].id;
		}
